                                        ...
                                        int width, int height)
{
    ...
    for(;height>=2;height -= 2) 
	{
        for(w = width; w >= 2; w -= 2) 
		{
            ...
            g1 = g;
            b1 = b;
            lum[0] = (uint8_t)RGB_TO_Y_CCIR(r, g, b);

            RGB_IN(r, g, b, p + BPP);
            r1 += r;
            g1 += g;
            b1 += b;
            lum[1] = (uint8_t)RGB_TO_Y_CCIR(r, g, b);
            p += wrap3;
            lum += wrap;
            ...
            cb[0] = (uint8_t)RGB_TO_U_CCIR(r1, g1, b1, 2);
            cr[0] = (uint8_t)RGB_TO_V_CCIR(r1, g1, b1, 2);
        ...
        if (w) {
            ...
            b1 += b;
            lum[0] = (uint8_t)RGB_TO_Y_CCIR(r, g, b);
            cb[0] = (uint8_t)RGB_TO_U_CCIR(r1, g1, b1, 1);
            cr[0] = (uint8_t)RGB_TO_V_CCIR(r1, g1, b1, 1);
            cb++;
            cr++;
            p += -wrap3 + BPP;
            lum += -wrap + 1;
    ...
    if (height) {
        ...
        if (w) {
            RGB_IN(r, g, b, p);
            lum[0] = (uint8_t)RGB_TO_Y_CCIR(r, g, b);
            cb[0] = (uint8_t)RGB_TO_U_CCIR(r, g, b, 0);
            cr[0] = (uint8_t)RGB_TO_V_CCIR(r, g, b, 0);
        }
    }
}
                              ...
                              int width, int height)
{
    ...
    for(;height>=2;height -= 2) {
        for(w = width; w >= 2; w -= 2) {
            ...
            g1 = g;
            b1 = b;
            lum[0] = (uint8_t)RGB_TO_Y(r, g, b);

            RGB_IN(r, g, b, p + BPP);
            r1 += r;
            g1 += g;
            b1 += b;
            lum[1] = (uint8_t)RGB_TO_Y(r, g, b);
            p += wrap3;
            lum += wrap;
            ...
            cb[0] = (uint8_t)RGB_TO_U(r1, g1, b1, 2);
            cr[0] = (uint8_t)RGB_TO_V(r1, g1, b1, 2);
        ...
        if (w) {
            ...
            b1 += b;
            lum[0] = (uint8_t)RGB_TO_Y(r, g, b);
            cb[0] =(uint8_t) RGB_TO_U(r1, g1, b1, 1);
            cr[0] = (uint8_t)RGB_TO_V(r1, g1, b1, 1);
            cb++;
            cr++;
            p += -wrap3 + BPP;
            lum += -wrap + 1;
    ...
    if (height) {
        ...
        if (w) {
            RGB_IN(r, g, b, p);
            lum[0] = (uint8_t)RGB_TO_Y(r, g, b);
            cb[0] = (uint8_t)RGB_TO_U(r, g, b, 0);
            cr[0] = (uint8_t)RGB_TO_V(r, g, b, 0);
        }
    }
}
