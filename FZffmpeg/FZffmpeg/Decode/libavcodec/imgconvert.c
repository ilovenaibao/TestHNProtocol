...
static void img_convert_init(void)
{
    ...
    for(i = 0;i < 256; i++) {
        y_ccir_to_jpeg[i] = (uint8_t)Y_CCIR_TO_JPEG(i);
        y_jpeg_to_ccir[i] =(uint8_t)Y_JPEG_TO_CCIR(i);
        c_ccir_to_jpeg[i] = (uint8_t)C_CCIR_TO_JPEG(i);
        c_jpeg_to_ccir[i] = (uint8_t)C_JPEG_TO_CCIR(i);
    }
}
